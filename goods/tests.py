...
def test_good_detail_api_get(good):
    resp = Client().get(f'/api/good/{good.pk}/')
    assert resp.status_code == 200
    assert resp.json()['name'] == 'Test Good'


def test_good_detail_api_post_without_csrf_token_is_forbidden(good):
    client = Client(enforce_csrf_checks=True)
    resp = client.post(f'/api/good/{good.pk}/', {'name': 'Renamed', 'price': '10.00'})
    assert resp.status_code == 403


def test_good_detail_api_post_with_csrf_token_saves(good):
    client = Client(enforce_csrf_checks=True)
